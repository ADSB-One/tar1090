...
if (window.location.href.match(/adsb.one/) && window.location.pathname == '/') {
    adsb_one = true;
}
...
if (feed != null) {
    ...
    if (split.length > 0) {
        ...
        if (uuid[0].length > 18) {
            ...
            let URL = 'https://www.adsb.one/api/feeders/tar1090/?feed=' + uuid[0];
            console.log(URL);
...
} else if (0 || (adsb_one && (hostname.startsWith('globe.') || hostname.startsWith('globe-')))) {
    console.log("Using adsb one fast-path load!");
    let data = {"zstd":true,"reapi":true,"refresh":1600,"history":1,"dbServer":true,"binCraft":true,"globeIndexGrid":3,"globeIndexSpecialTiles":[],"version":"adsb one backend"};
